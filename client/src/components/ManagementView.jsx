...
export function ManagementView({ currentUser, view, assets, peopleAssets = assets, query, onQueryChange, onNotify, onAddAsset, onAssetUpdated, onEditAsset, onDeleteAsset, audits, onScheduleAudit, maintenance, onLogMaintenance, onResolveMaintenance }) {
  const role = currentUser.role;
  if (view === 'Assets') return <AssetsView currentUser={currentUser} assets={assets} query={query} onQueryChange={onQueryChange} onAddAsset={onAddAsset} onNotify={onNotify} onAssetUpdated={onAssetUpdated} />;
  if (view === 'Audits') return <AuditView assets={assets} onNotify={onNotify} audits={audits} onScheduleAudit={hasPermission(role, 'scheduleAudits') ? onScheduleAudit : null} />;
...
function AssetsView({ currentUser, assets, query, onQueryChange, onAddAsset, onNotify, onAssetUpdated }) {
  ...
  return <><RequestAssetsView currentUser={currentUser} assets={assets} query={query} onQueryChange={onQueryChange} onNotify={onNotify}/><ApprovalRequestsView approverRole="Admin" assets={assets} onNotify={onNotify}/><AdminAssetsView role={currentUser.role} assets={assets} query={query} onQueryChange={onQueryChange} onAddAsset={onAddAsset} onNotify={onNotify} onAssetUpdated={onAssetUpdated}/></>;
}

function AdminAssetsView({ role, assets, query, onQueryChange, onAddAsset, onNotify, onAssetUpdated }) {
  ...
  const saveEditedAsset = (asset) => { setManagedAssets((current) => current.map((item) => item.assetId === asset.assetId ? asset : item)); onAssetUpdated?.(asset); window.dispatchEvent(new CustomEvent('assetflow-asset-updated', { detail: asset })); setEditingAsset(null); onNotify(`${asset.assetId} updated successfully`); };
...
function AuditView({ assets, onNotify, audits, onScheduleAudit }) {
  const completedAudits = audits.filter((audit) => audit.status === 'Completed').length;
  const auditProgress = audits.length ? Math.round((completedAudits / audits.length) * 100) : 0;
  const exceptions = [
    ['Missing or lost assets', assets.filter((asset) => asset.status === 'Lost').length, 'All locations'],
    ['Assets in maintenance', assets.filter((asset) => asset.status === 'Maintenance').length, 'All locations'],
    ['Unassigned assets', assets.filter((asset) => !asset.assignedTo && asset.status !== 'Retired').length, 'All locations']
  ];
