...
const initialForm = { name: '', purchaseCost: '', model: '', category: 'Computer', location: 'Johannesburg HQ', status: 'Available' };

export function AssetForm({ asset, onClose, onSave }) {
  const [form, setForm] = useState(asset ? { ...initialForm, ...asset } : initialForm);
  ...
  const submit = async (event) => {
    ...
    try {
      const response = await fetch(asset?._id ? `/api/assets/${asset._id}` : '/api/assets', {
        ...
        body: JSON.stringify({ ...form, assetId: asset?.assetId || `AST-${Date.now().toString().slice(-4)}` })
      });
